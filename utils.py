...
def find_optimal_params_for_range(
    feature_matrix: np.ndarray,
    umap_cluster_dims: int,
    min_clusters: int,
    max_clusters: int,
) -> dict | None:
    """Grid-search HDBSCAN params that produce n_clusters in [min_clusters, max_clusters].

    Returns dict with min_cluster_size, min_samples, cluster_epsilon, n_clusters,
    silhouette, davies_bouldin — maximising silhouette within the target range.
    Returns None if no combination qualifies.
    """
    n = len(feature_matrix)
    actual_dims = min(umap_cluster_dims, n - 2, feature_matrix.shape[1])
    reducer = umap.UMAP(
    ...
    sizes = [s for s in [2, 3, 5, 7, 10, 15, 20, 30] if s <= max(2, n // 2)]
    if not sizes:
        sizes = [2]
    ...
    for mcs in sizes:
        for ms in [1, 2, 3, 5]:
            if ms > mcs:
                continue
            for eps in [0.0, 0.1, 0.2, 0.5, 1.0]:
                try:
                    labels = hdbscan.HDBSCAN(
                        min_cluster_size=mcs,
                        min_samples=ms,
                    ...
                    nc = len([l for l in set(labels) if l >= 0])
                    if not (min_clusters <= nc <= max_clusters):
                        continue
                    mask = labels != -1
                    if nc < 2 or mask.sum() < 2:
                        continue
