...
with st.sidebar:
    ...
    # Nav items with step numbers and subtitles
    _nav_items = [
        ("01", "Setup",           "API key · data · dimensions"),
        ("02", "Embed & Cluster", "Embeddings · HDBSCAN"),
        ("03", "Review & Edit",   "Inspect · rename · chat"),
        ("04", "Analytics",       "Metrics · ranking · export"),
    ]
    for _num, _label, _sub in _nav_items:
        _is_done = (
            (_num == "01" and bool(st.session_state.get("api_key")) and _has_data) or
            (_num == "02" and _has_embeddings) or
            (_num == "03" and _confirmed) or
            (_num == "04" and _confirmed)
        )
        _dot_color = "#26B4D2" if _is_done else "#516e81"
        st.markdown(
            f"""
            <div style="display:flex;align-items:flex-start;gap:10px;
                 padding:7px 4px;margin-bottom:2px">
              <div style="font-size:9px;font-weight:700;color:{_dot_color};
                   letter-spacing:0.05em;padding-top:2px;flex-shrink:0">{_num}</div>
              <div>
                <div style="font-size:12px;font-weight:600;color:#d0dce6">{_label}</div>
                <div style="font-size:10px;color:#516e81">{_sub}</div>
              </div>
            </div>
            """,
            unsafe_allow_html=True,
